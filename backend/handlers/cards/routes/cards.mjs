...
import { createCardObject, updatedCardObject } from "../model/cardModel.mjs";
...
// Get deleted user cards
router.get("/deleted-cards", guard, async (req, res) => {
  try {
    const user = getUser(req);
    const cards = await getDeletedUserCards(user._id);
    res.send(cards);
  } catch (error) {
    return handleError(res, error.status || 500, error.message);
  }
...
// Restore card
router.patch("/restore-card/:id", guard, async (req, res) => {
  try {
    const cardId = req.params.id;
    const restoredCard = await restoreCard(cardId);
    res.send(restoredCard);
  } catch (error) {
    return handleError(res, error.status || 500, error.message);
  }
});
...
router.put("/:id", guard, async (req, res) => {
  try {
    ...
    const updatedCardData = updatedCardObject(req);
    const user = getUser(req);
...
router.delete("/:id", guard, async (req, res) => {
  try {
    const user = getUser(req);
    const card = await deleteCard(req.params.id, user._id, user.isAdmin);
    res.send(card);
  } catch (error) {
    return handleError(res, error.status || 500, error.message);
  }
